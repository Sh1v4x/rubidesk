...
export type ElementChoice = "normal" | "air" | "fire" | "shushu" | "auto";
...
export function parseElement(text: string): ElementChoice | null {
  ...
  if (/\b(monstre|shushu|colosse|vraie forme|epee)\b/.test(norm)) {
    // « sors de l'épée », « forme shushu », « transforme-toi en monstre »
    return /\bepee\b/.test(norm) && !/\bsors\b/.test(norm) ? null : "shushu";
  }
